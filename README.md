...
> 三层的**失效边界是分开设计的**：当 Hub（边） 不可用时、设备（端）仍能访问互联网时直连兜底；
> **云这层挂了也照旧能在群里 @ 设备机器人 直接控制** —— 不是简单堆三层。
...
10. **可接入 AI Agent（可选，纯增量）**：把企微群里的话交给**你自己的 AI Agent** 去理解与规划 ——
    它替你 @ 设备机器人、**核对每条指令是否真的执行了**（被丢弃的还会重发）。
    于是「控制」从"背指令"变成"说人话"。**今天就能用：本仓库的服务与固件一行都不用改。**
    教程（不给成品代码，harness 不限）：[`ai-agent-wecom/`](./ai-agent-wecom/README.md)
...
> 它去 @ 设备机器人 并**逐条核对回执**，被丢弃的还会重发。
> 接入教程：[`ai-agent-wecom/`](./ai-agent-wecom/README.md)（**纯增量**，本仓库的服务一行都不用改）
...
- [x] **AI Agent 接入企微**（自然语言 → 指令；同一套解耦可扩展到新设备，教程见 `ai-agent-wecom/`）

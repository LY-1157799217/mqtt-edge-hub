...
> 这是面向**受信任局域网**的桌面信息显示与设备控制原型。树莓派提供数据接口、缓存和行情阈值监控，
> 通过本机 MQTT 总线连接企业微信机器人与指令执行器；树莓派不可达且设备仍能访问互联网时，
> ESP32-C3「小电视」可**直连上游兜底取数并显示**（协议分路，见亮点 5）。
>
> 项目还提供**可选的 AI Agent 接入指南**：接入后，可通过企业微信提出自然语言需求，
> 由 Agent 拆解操作步骤，经现有控制链路下发指令，并读取执行日志核对结果，
> 形成端、边与 AI 服务协同的控制流程。具体 Agent 工具与框架适配需自行配置或实现，
> 详见 [AI Agent 接入教程](./ai-agent-wecom/README.md)。
>
> 本仓库**同时包含两侧代码**：Pi（Linux）侧在仓库根目录，ESP32 侧固件在 `firmware/`；
> `ai-agent-wecom/` 提供接入教程与设备操作规程，**不含成品 Agent 实现**。
...
> **端‑边‑云三层协同**：**端**（ESP32 只管显示，树莓派不可达时**自己直连上游兜底**）；
> **边**（树莓派做缓存 / 告警 / 指令总线，也是"降级"的落点）；
> **云**（可选接入 AI Agent，**Agent 经控制中介下发指令，不直接访问设备接口**）。
> 三层的**失效边界是分开设计的**：当 Hub（边）不可用、而设备（端）仍能访问互联网时，直连兜底；
> **AI 服务不可用、企业微信与本地控制链路仍正常时，仍可在群里 @ 设备机器人直接控制。**
...
10. **可接入 AI Agent（可选）**：把企微群里的自然语言需求交给**你自己的 AI Agent** 理解与规划。
    完成发送工具与日志读取能力适配后，Agent 可拆解操作步骤，经设备机器人下发指令、
    **核对执行回执**，并按操作规程处理设备忙与结果未知等情况。
    成功回执表示控制请求已受理，**不等于已确认屏幕完成变化**。
    接入可复用现有服务与固件；本仓库提供框架无关的教程与操作规程，**不含成品 Agent 实现**：
    [`ai-agent-wecom/`](./ai-agent-wecom/README.md)
...
ai-agent-wecom/   可选教程：把 AI Agent 接入企微，用自然语言控制设备（不含成品代码）
  README.md               教程主体（原理 + 三步操作 + 实拍效果）
  esp32-agent-brief.md    给读者 AI Agent 的「设备操作规程」（整段复制粘贴即可）
  config.example.md       脱敏配置片段
  assets/                 教程配图
...
> 💡 **不想记这些指令？** 可以让一个 **AI Agent** 替你说 —— 你在群里 @ 它说人话，
> 完成发送工具与日志读取能力适配后，它可经设备机器人下发指令、**逐条核对回执**，
> 并按操作规程处理设备忙与结果未知等情况。
> 接入教程：[`ai-agent-wecom/`](./ai-agent-wecom/README.md)（复用现有控制链路，**不含成品 Agent 实现**）

## 符号规则

- 股票 / 指数：`sh600519` `sz000001` `sh000001`（上证指数）
- ⚠️ **场外基金（`f005827` 这类）不支持** —— 上游是腾讯股票行情接口，该接口对基金代码
  不返回数据（实测返回"无匹配"）。**无Pi版的主工程同样不支持**，两版在这一点上**是一致的**。
...
- [x] **AI Agent 接入企微教程**（自然语言 → 指令；提供接入契约与操作规程，具体工具需自行适配，见 [`ai-agent-wecom/`](./ai-agent-wecom/README.md)）
...
> 📖 **想看「AI Agent 接入企微」的实际效果图？** → [`ai-agent-wecom/assets/`](./ai-agent-wecom/assets/)
> （4 张实测截图：必须 @ 才生效、一句话拆多步、发现被丢弃的指令并提出补发、问设备之外的问题）

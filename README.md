...
5. **设备侧自动兜底**：树莓派不可达时，小电视**自己直连上游**（HTTPS + 跳过证书校验），
   并在满足条件时**补取分时图**（`DIRECT_SPARK_BACKFILL`）
...
9. **AI‑Native 人机跨系统协同开发**：与无Pi版同一套工作流 —— AI 承担代码生成、排错、文档，
   **人主导需求决策、BUG 定位、硬件实物验证、业务逻辑把关**
10. **跨操作系统 + 多智能体协同**：Windows + Linux 跨系统协同，构建多 AI Agent 协同方案，
    人工审核信息、同步进度、方向把控
...
acceptance_s9.sh  验收自检脚本（baseline/check 两模式）
...
> - **ST 股（±5%）、北交所（±30%）未做适配** —— 前者永不触发，后者阈值不准；
...
- Flask 绑 `0.0.0.0` 且**没有任何认证** —— **绝不要把 5000 端口转发到公网**，
